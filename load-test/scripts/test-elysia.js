...
/**
 * Script de teste de carga otimizado para Elysia
 * 
 * Variáveis de ambiente:
 * - ELYSIA_URL: URL do servidor (padrão: http://localhost:3001)
 ...
 * Exemplos de uso:
 *   npm run test:elysia
 *   CONCURRENCY=200 npm run test:elysia
 *   CONCURRENCY=50 BATCH_SIZE=500 npm run test:elysia
 *   TOTAL_REQUESTS=10000 CONCURRENCY=50 npm run test:elysia
 */
...
async function runTest() {
  console.log('🚀 Iniciando teste de carga para Elysia (Bun.js)...\n');
  ...
  if (!healthCheck.success) {
    ...
    console.error(`   1. cd api-bunjs-elysia`);
    console.error(`   2. bun run dev`);
    console.error(`\n   Ou verifique se o servidor está rodando em outra porta.`);
    console.error(`   Você pode definir a URL com: ELYSIA_URL=http://localhost:PORTA node scripts/test-elysia.js\n`);
    process.exit(1);
  }
  ...
  console.log(`   CONCURRENCY=200 BATCH_SIZE=2000 npm run test:elysia`);
  console.log(`   TOTAL_REQUESTS=10000 CONCURRENCY=50 npm run test:elysia\n`);
  ...
  // Conectar os streams: Readable -> Transform -> Writable
  requestGenerator
